...
const text = {
  ...
  pluggedInStatus: (status) => (status ? 'Yes' : 'No'),
  state: 'Current state',
  stateStatus: (status) => formatName(status),
  timeToCompletion: 'Time to completion',
  timeToCompletionStatus: (status) => timeDiff(status),
  battery: 'Battery level',
  batteryStatus: (status) => `${status * 100}%`,
  range: 'Range',
  rangeStatus: (status) => `${status} miles`,
  chargeLimit: 'Charge limit',
  chargeLimitStatus: (status) => `${status * 100}%`,
  capacity: 'Capacity',
  capacityStatus: (status) => `${status} kilowatt-hours`,
  voltage: 'Voltage',
